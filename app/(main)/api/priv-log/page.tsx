...
export default function LogsPage() {
  ...
  const tTarget = (key?: string) =>
    (key &&
      ACTIVITY_LOGS_LABELS.targetNames[
        key as keyof typeof ACTIVITY_LOGS_LABELS.targetNames
      ]) ||
    key ||
    "-";
  ...
  const tAction = (key?: string) =>
    (key &&
      ACTIVITY_LOGS_LABELS.actionNames[
        key as keyof typeof ACTIVITY_LOGS_LABELS.actionNames
      ]) ||
    key ||
    "-";
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        const data = await api.get<ActivityLogType>(
          ENDPOINTS.USERS.ACTIVITY_LOGS(page)
        );
          ...
          {logData?.data?.map((item, index) => (
            ...
            >
              ...
              <td className="p-2 flex gap-2">
                {" "}
                {item.target_id ? (
                  <div className="text-green-500">{item.target_id}#</div>
                ) : (
                  ""
                )}
                {tTarget(item.target_type)} {tAction(item.action)}
              </td>
      ...
      {logData?.pagination && (
        <div className="flex items-center justify-between mt-4">
          <span className="text-sm text-gray-400">
            Total: {logData.pagination.total} | Page {logData.pagination.page}{" "}
            of {logData.pagination.totalPages}
          </span>
